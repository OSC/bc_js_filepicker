...
function extract_favorites_from_document(doc) {
  ...
  for (let element of doc
    .querySelectorAll('li[title="Files"] a[title]')
    .values()) {
    let { title, href } = element

    // Interactive regex explorer:
    // https://regex101.com/r/pkOLR3/1/
    let extractPath = new RegExp('pun/(sys|dev)/(ood-)?dashboard/files/fs(.*)')
...
export function list_path(path) {
  const fs_api_root = '/pun/sys/dashboard/files/api/v1/fs/'

  return fetch(
    location.origin + pathmod.resolve(fs_api_root, path.replace(/(^\/)/, '')),
    {
...
export function file_system_favorites_from_input_data(input) {
  ...
  try {
    ...
    console.error(
      'Unable to use favorites from the data attribute because: ' + error,
    )
    return false
  }
}
